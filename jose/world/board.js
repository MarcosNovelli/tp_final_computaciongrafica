...
function createBoard(boardWidth = 5, boardHeight = 5, baseNoiseGenerator = null) {
  ...
  function getTileHexSize() {
    // CORRECCIÓN PARA FLAT-TOP: Los tiles deben tocarse por los LADOS (no por las puntas)
    // El tamaño debe asegurar que los lados de los tiles se toquen exactamente
    // sin dejar espacios negros entre ellos
    ...
    // Distancia del centro del tile al centro del hexágono más lejano
    // Para flat-top, la distancia vertical entre centros es: size * sqrt(3) * distancia_hexagonal
    // El hexágono más lejano está a distancia hexagonal GRID_RADIUS
    // Entonces: centerToCenterDistance = HEX_RADIUS_WORLD * sqrt3 * GRID_RADIUS
    const centerToCenterDistance = HEX_RADIUS_WORLD * sqrt3 * GRID_RADIUS;
    
    // Apotema del hexágono individual (distancia del centro al borde plano)
    // Para flat-top, el borde es plano, así que usamos el apotema
    const hexApothem = HEX_RADIUS_WORLD * sqrt3 / 2;
    
    // Radio total del tile (centro del tile al borde plano del hexágono más lejano)
    const tileRadius = centerToCenterDistance + hexApothem;
    
    // Altura total del tile (borde plano a borde plano opuesto)
    // Para flat-top, esto es: 2 * tileRadius
    const tileHeight = 2 * tileRadius;
    
    // Para que hexToPixel3D (flat-top) produzca esta distancia entre centros de tiles:
    // La fórmula flat-top usa: z = size * sqrt(3) * (r + q/2)
    // Para tiles adyacentes verticalmente (mismo q, r diferente):
    //   spacing = tileHexSize * sqrt(3)
    // Para que spacing = tileHeight:
    //   tileHexSize * sqrt(3) = tileHeight
    //   tileHexSize = tileHeight / sqrt(3)
    const tileHexSize = tileHeight / sqrt3;
  ...
  function calculateTileOffset(qTile, rTile) {
    // Reutilizar la misma función que posiciona hexágonos pequeños
    // Esta función ya está diseñada para orientación pointy-top, que es la que
    // usan los hexágonos individuales dentro de cada tile
    // Solo cambia el tamaño: tileHexSize en lugar de HEX_RADIUS_WORLD
    const tileHexSize = getTileHexSize();
    const pos = hexToPixel3D(qTile, rTile, tileHexSize);
    
    // Verificación: hexToPixel3D ya usa la fórmula correcta para pointy-top:
    // x = size * sqrt(3) * (q + r/2)
    // z = size * 1.5 * r
    // Esta es la fórmula estándar que garantiza encaje perfecto para hexágonos pointy-top
  ...
  function generate() {
    ...
    // Para un tablero 5×5, usamos radio hexagonal de 2
    // Esto genera aproximadamente 25 tiles (1 + 6 + 18)
    const BOARD_RADIUS = 2;
    const tileHexSize = getTileHexSize();
    const sqrt3 = Math.sqrt(3);
    // Radio del tile: desde el centro hasta el borde del hexágono más lejano
    // Incluye el radio del hexágono individual para evitar huecos
    const tileRadius = HEX_RADIUS_WORLD * sqrt3 * GRID_RADIUS + HEX_RADIUS_WORLD;
    const tileDiameter = 2 * tileRadius;
    const tileSpacing = tileHexSize * sqrt3; // Distancia entre centros de tiles adyacentes
    
    console.log(`✓ Creando tablero hexagonal tipo Catan (radio: ${BOARD_RADIUS})`);
    console.log(`  - Tamaño de un hexágono pequeño: ${HEX_RADIUS_WORLD.toFixed(2)}`);
    console.log(`  - Radio de un tile (en celdas): ${GRID_RADIUS}`);
    console.log(`  - Radio de un tile (centro a borde): ${tileRadius.toFixed(2)}`);
    console.log(`  - Diámetro de un tile (borde a borde): ${tileDiameter.toFixed(2)}`);
    console.log(`  - Tamaño de hexágono en grilla de tiles: ${tileHexSize.toFixed(2)}`);
    console.log(`  - Distancia entre centros de tiles adyacentes: ${tileSpacing.toFixed(2)}`);
    console.log(`  - Verificación: distancia = diámetro? ${Math.abs(tileSpacing - tileDiameter) < 0.01 ? '✓ SÍ' : '✗ NO (diferencia: ' + Math.abs(tileSpacing - tileDiameter).toFixed(3) + ')'}`);
    
    // Generar tiles en una grilla hexagonal completamente llena
    // Para asegurar que no haya espacios negros, iteramos sobre un rango más amplio
    // y generamos todos los tiles dentro del radio hexagonal
    // Esto garantiza que todos los espacios internos estén llenos
    for (let qTile = -BOARD_RADIUS - 1; qTile <= BOARD_RADIUS + 1; qTile++) {
      for (let rTile = -BOARD_RADIUS - 1; rTile <= BOARD_RADIUS + 1; rTile++) {
        // Calcular distancia hexagonal desde el centro (0, 0)
        const distance = hexDistance(0, 0, qTile, rTile);
        
        // Solo generar tiles dentro del radio especificado (estricto para mantener forma)
        if (distance > BOARD_RADIUS) {
          continue; // Salta este tile si está fuera del radio
        }
        
        // Asignar bioma al tile (aleatorio uniforme)
        const biome = assignBiomeToTile(qTile, rTile);
        
        // Crear generador de ruido único para este tile (con offset)
        // Esto asegura que tiles del mismo bioma tengan relieves diferentes
        const tileNoiseGenerator = createTileNoiseGenerator(qTile, rTile, baseNoiseGenerator);
        
        // Calcular offset de posición usando coordenadas hexagonales
        // Esto posiciona el tile como un hexágono grande en la grilla hexagonal
        const offset = calculateTileOffset(qTile, rTile);
        
        // Crear el tile con su generador de ruido único y su offset de posición
        const tile = createTile(biome, offset.x, offset.z, tileNoiseGenerator);
    ...
    generated = true;
    console.log(`✓ Tablero generado: ${tiles.length} tiles creados en grilla hexagonal tipo Catan`);
  }
  ...
  function getBoardSize() {
    const BOARD_RADIUS = 2; // Radio del tablero 5×5
    const tileHexSize = getTileHexSize();
    // El diámetro de un tile (de borde a borde opuesto)
    const tileDiameter = 2 * GRID_RADIUS * HEX_RADIUS_WORLD * Math.sqrt(3);
    // Calcular el tamaño del tablero: distancia del centro al tile más lejano * 2
    // El tile más lejano está en radio BOARD_RADIUS, y su distancia al centro es aproximadamente:
    const boardSize = BOARD_RADIUS * tileDiameter * 1.5; // Factor 1.5 para cubrir el área completa
    return {
      width: boardSize,
      height: boardSize
    };
